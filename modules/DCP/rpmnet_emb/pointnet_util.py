...
def sample_and_group_multi(npoint: int, radius: float, nsample: int, xyz: torch.Tensor, normals: torch.Tensor,
                           returnfps: bool = False, is_centerfeat: bool = False):
    ...
    grouped_xyz = index_points(xyz, idx)  # (B, npoint, nsample, C)
    d = grouped_xyz - new_xyz.view(B, S, 1, C)  # d = p_r - p_i  (B, npoint, nsample, C)
...
def sample_and_group_multi_center(npoint: int, radius: float, nsample: int, xyz: torch.Tensor, normals: torch.Tensor,
                           returnfps: bool = False, is_fake_center=False):
    """Sample and group for xyz, dxyz and ppf features
    ...
    if not is_fake_center:
        nr_dd0 = angle(nr, new_xyz[:, :, None, :]).expand(-1, -1, grouped_xyz.shape[2])  # (B, npoint, 1)
        nr_dd1 = angle(nr, grouped_xyz)  # (B, npoint, n_sample)
        ni_dd = angle(ni, grouped_xyz)  # (B, npoint, n_sample)
        d_norm1 = torch.norm(new_xyz, dim=-1, keepdim=True).expand(-1, -1, grouped_xyz.shape[2])  # (B, npoint, 1)
        d_norm2 = torch.norm(grouped_xyz, dim=-1)  # (B, npoint, nsample)
        centerfeat = torch.stack([nr_dd0, nr_dd1, ni_dd, d_norm1, d_norm2], dim=-1)  # (B, npoint, n_sample, 5)
    else:
        fake_center = xyz.mean(1, keepdim=True)[:, :, None, :]  # [B, 1, 1, 3]
        nr_dd0 = angle(nr, new_xyz[:, :, None, :] - fake_center).expand(-1, -1, grouped_xyz.shape[2])  # (B, npoint, 1)
        nr_dd1 = angle(nr, grouped_xyz - fake_center)  # (B, npoint, n_sample)
        ni_dd = angle(ni, grouped_xyz - fake_center)  # (B, npoint, n_sample)
        d_norm1 = torch.norm(new_xyz - fake_center.squeeze(2), dim=-1, keepdim=True).expand(-1, -1, grouped_xyz.shape[2])  # (B, npoint, 1)
        d_norm2 = torch.norm(grouped_xyz - fake_center, dim=-1)  # (B, npoint, nsample)
        centerfeat = torch.stack([nr_dd0, nr_dd1, ni_dd, d_norm1, d_norm2], dim=-1)  # (B, npoint, n_sample, 5)


    if returnfps:
        return {'xyz': new_xyz, 'dxyz': xyz_feat, 'ppf': ppf_feat, "centerfeat": centerfeat}, grouped_xyz, fps_idx
    else:
        return {'xyz': new_xyz, 'dxyz': xyz_feat, 'ppf': ppf_feat, "centerfeat": centerfeat}

# def sample_and_group_multi_center(npoint: int, radius: float, nsample: int, xyz: torch.Tensor, normals: torch.Tensor, returnfps: bool = False):
#     """Sample and group for xyz, dxyz and ppf features
#
#     Args:
#         npoint(int): Number of clusters (equivalently, keypoints) to sample.
