...
class DCP(nn.Module):
    def __init__(self, args):
        ...
        elif args.emb_nn == "rpmnet_emb":
            from modules.DCP.rpmnet_emb.feature_net import FeatExtractionEarlyFusion
            self.emb_nn = FeatExtractionEarlyFusion(features=args.features, feature_dim=args.feat_dim,
    ...
    def forward(self, data):
        ...
        src_pcd = data["src_pcd"].transpose(2, 1).contiguous()    # [B, 3, N]
        tgt_pcd = data["model_pcd"].transpose(2, 1).contiguous()  # [B, 3, M]
        if self.args.emb_nn in ["rpmnet_emb"]:
            src_pcd_norm = data["src_pcd_normal"]
            tgt_pcd_norm = data["model_pcd_normal"]
            src_embedding = self.emb_nn(src_pcd.transpose(2, 1), src_pcd_norm).transpose(2, 1).contiguous()
            tgt_embedding = self.emb_nn(tgt_pcd.transpose(2, 1), tgt_pcd_norm).transpose(2, 1).contiguous()
        elif self.args.emb_nn in ["pointnet", "dgcnn"]:
            src_embedding = self.emb_nn(src_pcd)
            tgt_embedding = self.emb_nn(tgt_pcd)
        else:
        ...
        Rs_pred, ts_pred = self.head(src_embedding, tgt_embedding, src_pcd, tgt_pcd)
        return Rs_pred, ts_pred